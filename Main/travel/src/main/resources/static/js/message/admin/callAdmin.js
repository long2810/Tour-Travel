...
callSystem();
function callSystem(){
    const pathSplitAdmin= location.pathname.split("/");
    const calleeId = pathSplitAdmin[pathSplitAdmin.length-1];
        ...
        <div id="videoBox" class="d-none ms-5 me-5 mt-3 border border-4 rounded bg-light">
            ...
            <div class="p-3 d-flex flex-wrap">
                <div class="d-flex w-100 justify-content-center p-3 gap-3">
                    <div id="remote-display" class="d-none col-12 col-lg-6 border bg-white rounded" style="height: 350px;">
                        <video id="remote-video" style="transform: scaleX(-1);" class="h-100 w-100 rounded" autoplay playsinline></video>
                    </div>
                    <div id="local-display" class="col-12 col-lg-9 border bg-white rounded" style="height: 350px;">
    ...
    const videoCall = document.getElementById("start-call");
    ...
    videoCall.addEventListener("click", e=>{
        ...
        startCall(setupCall, {calleeId: calleeId, callerId: userLogin.id, video: true }, localVideo, configuration, stompClientCall);
    ...
    stompClientCall.onConnect=(frame)=>{
        ...
        stompClientCall.subscribe("/topic/reject", (callResponse)=>{
            ...
            if (userLogin.id===calling.callerId && calling.status==="Reject"){
                ...
                alert(`${calling.calleeName}님은 바쁜 중입니다`);
            }
        })
